...
pub struct MacosDeploymentScriptInput<'a> {
    pub server_config: &'a ServerConfig,
    pub enrollment_token: &'a str,
    pub opendesk_base_url: &'a str,
}

pub fn render_macos_deployment_script(input: &MacosDeploymentScriptInput<'_>) -> String {
    format!(
        r#"#!/usr/bin/env bash
set -euo pipefail
...
if ! command -v rustdesk >/dev/null 2>&1; then
  echo "rustdesk client is required; install the official macOS package first" >&2
  exit 1
fi
...
mod tests {
    ...
    #[test]
    fn render_macos_deployment_script_includes_server_values() {
        let config = default_server_config();
        let script = render_macos_deployment_script(&MacosDeploymentScriptInput {
