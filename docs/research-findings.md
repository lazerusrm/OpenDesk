...
| R-006 | Released Linux `.deb` validated `--deploy --token` against a controlled dev endpoint. The client posts bearer-auth JSON to `/api/devices/deploy` with device ID, UUID, and public key, and accepts `{"result":"OK"}`. | Test Windows/macOS clients and implement isolated compatibility adapter tests for `OK`, `NOT_ENABLED`, `INVALID_INPUT`, and `ID_TAKEN`. |
